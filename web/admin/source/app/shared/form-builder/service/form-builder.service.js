		...
		.factory('fromBuilderService', function() {
			return {
				...
				getTemplateByElement: function(element) {
					...
					switch (element.type) {
						...
						case 'choice':
							...
							$('label', elementTemplate).attr('translate', element.translateLabel).text(element.label);
							break;
					}
				...
				addOptionsToInputElement: function(el, template) {
					...
					$('label', template).attr('for', el.value).attr('translate', el.translateLabel)
					                    .text(el.label);
					return template;
				},
						...
						if (angular.isObject(formEl[el]['children'])) {
							for (var elem in formEl[el]['children']) {
								...
								elements[currEl.name] = {
									...
									label: currEl.label,
									emptyValue: currEl.empty_value,
									translateLabel: currEl.translateLabel
								}
							}
						}
