...
class ApplicationEventListenerServiceTest extends BaseController
{
	...
	public function testApplicationEventListener()
	{
		...
		$this->assertTrue(count($dispatcher->getListeners(ApplicationEvent::POST_PERSIST)) > 0);
		$this->assertTrue(count($dispatcher->getListeners(ApplicationEvent::POST_UPDATE)) > 0);
		$this->assertTrue(count($dispatcher->getListeners(ApplicationEvent::POST_REMOVE)) > 0);
	}
}