...
class ApplicationRepository extends EntityRepository
{
    ...
    public function getApplicationStatusesDayly()
    {
        ...
        $qb
            ->select('a.name')
            ->addSelect('ROUND(SUM(CAST(CASE WHEN l.status = :errors THEN 1 ELSE 0 END AS NUMERIC)) / COUNT(a.id), 4) * 100 AS errors')
            ->addSelect('ROUND(SUM(CAST(CASE WHEN l.status = :problems  THEN 1 ELSE 0 END AS NUMERIC)) / COUNT(a.id), 4) * 100 AS problems')
            ->addSelect('ROUND(SUM(CAST(CASE WHEN l.status = :success  THEN 1 ELSE 0 END AS NUMERIC)) / COUNT(a.id), 4) * 100 AS success')
            ->addSelect('ROUND(SUM(CAST(CASE WHEN l.status = :disabled THEN 1 ELSE 0 END AS NUMERIC)) / COUNT(a.id), 4) * 100 AS disabled')
